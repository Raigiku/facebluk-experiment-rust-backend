...
pub async fn handle(
    ...
    let es_user = event_store.user_q_find_by_id(&req.user_id).await?;
    if es_user.is_none() {
        ...
        event_store.user_m_register(&user_registered).await?;
    }

    let ua_user = user_auth.user_q_find_by_id(&req.user_id).await?;
    if let Some(ref user) = ua_user {
        if user.is_registered() {
            user_auth
                .user_m_mark_as_registered(&req.user_id, &DateTime::now())
                .await?;
        }
    }
